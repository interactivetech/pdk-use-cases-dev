...
Date: 09.18.2023
Description: Adapted train.py that removes using the continue_experiment() call in line 196
Reason: when using continue_experiment function: PytorchTrial by default load previous checkpoint.
This fails when you instantiate a new model with different number of classes (because new number of classes changes 
...
def get_or_create_model(client, model_name, pipeline, repo):
    # update to use get_model rather than get_models
    models=[]
    try:
...
def create_model(client, model_name, pipeline, repo):
    '''
    Andrew(9.18.2023): Stub that doesnt check if model exists in registry, but creates one
